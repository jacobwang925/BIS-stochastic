...
def roc_curve_from_data(models, settings, passive = True):
    ...
    for model in models:
        ...
        for args in settings:
            ...
            cpf = open('drawing_data/'+model+'_'+args[0]+'_'+str(passive)+'_nonoise','rb')
            cp = pickle.load(cpf)
        ...
        # plt.xlim(-20, 0)
            ...
            # data for noisy systems
            cpf = open('drawing_data/' + model + '_' + args[0] + '_' + str(passive)+'_noise', 'rb')
            cp = pickle.load(cpf)
            ...
            s = [20 * (abs(x) < 1e-9) for x in collision]
            auc = sum([(efficiency[i] + efficiency[i + 1]) * (safety[i + 1] - safety[i]) / 2 for i in
                       range(first_safe, len(collision) - 1)])
            ...
            plt.scatter(safety[idx], efficiency[idx], label='_nolegend_', c='C' + str(c), s=20)
            mask = np.ones(len(safety))
            mask[idx] = 0
            rest = np.where(mask)[0]
            # plt.scatter(safety[rest], efficiency[rest], c='C' + str(c), alpha=.2, s=20, linewidth=0)
        ...
        # tikz_save(model+'.tex')
        save_name = model+'_comparison.pdf'
        if not passive:
...
if __name__ == "__main__":
    ...
    settings = [ \
        ...
        ('StoSafe', {'d_min': [1, 1.5, 2], 'alpha': [0.2, 0.5, 0.8, 1], 'epsilon': [0.1, 0.2, 0.3], 'algo': [1]}), \
    ...
    roc_curve_from_data(models, settings)