...
class KinematicModel:
    ...
    def __init__(self, init_state, agent, dT, auto, is_2D=False):
        ...
        # self.control_noise = 0.2  # noise scale
        self.control_noise = 1  # noise scale
        # self.control_noise = 2  # noise scale
        self.safe_dis = 1
        ...
        # self.measure_noise = 0.2  # noise scale
        self.measure_noise = 1  # noise scale
        # self.measure_noise = 2  # noise scale
        self.auto=False # whether is controlled by human
    ...
    def update(self, obstacle, Sto = False, MC = True, F = None):
        ...
        if Sto:
            self.calc_control_sto(obstacle, MC, F)
        else:
            self.calc_control(obstacle)
        # self.calc_control(obstacle, F)
        self.update_trace()
    ...
    def calc_control_sto(self, obstacle, MC = True, F = None):
        """
        Generate control input by the agent.
        ...
        max_u = self.max_u
        if MC:
            self.u = self.agent.calc_control_input(dT, goal, fx, fu, Xr, Xh, dot_Xr, dot_Xh, Mr, Mh, p_Mr_p_Xr,
                                                   p_Mh_p_Xh, u0, min_u, max_u)
        else:
            self.u = self.agent.calc_control_input(dT, goal, fx, fu, Xr, Xh, dot_Xr, dot_Xh, Mr, Mh, p_Mr_p_Xr, p_Mh_p_Xh, u0, min_u, max_u, MC, F)
        self.u = self.filt_u(self.u)
